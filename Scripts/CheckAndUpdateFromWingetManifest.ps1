...
function Write-Log {
    ...
    # Redact GitHub tokens (PAT format)
    $sanitizedMessage = $sanitizedMessage -replace 'github_pat_[a-zA-Z0-9_]{22,}', 'REDACTED_TOKEN'
    $sanitizedMessage = $sanitizedMessage -replace 'ghp_[a-zA-Z0-9]{36,}', 'REDACTED_TOKEN'
    
    # Redact common token references
    $sanitizedMessage = $sanitizedMessage -replace '(token|access[_\s]token|api[_\s]key)(["\s:=]+)["\'']?[a-zA-Z0-9_\-\.]{30,}["\'']?', '$1$2"REDACTED"'
    
    # Remove any other long alphanumeric strings that might be tokens
    if ($sanitizedMessage -match '--token\s+[a-zA-Z0-9_\-\.]{20,}') {
        $sanitizedMessage = $sanitizedMessage -replace '(--token\s+)[a-zA-Z0-9_\-\.]{20,}', '$1***'
    }
    
    # Remove ALL colons from log messages as requested - more aggressive replacement
...
function Fix-KomacManifestsAndSubmit {
    ...
    $Env:KOMAC_LOG_LEVEL = "INFO"  # Prevent Komac from logging sensitive debug data
    ...
    if ($GitHubToken) {
        try {
            # Don't log the command with token
            Write-Log "Updating Komac GitHub token" -Level DEBUG
            
            # Run token update without capturing or displaying output
            $null = & $KomacPath token update --token $GitHubToken 2>&1
            
            if ($LASTEXITCODE -eq 0) {
                Write-Log "Successfully updated Komac GitHub token" -Level DEBUG
            }
            else {
                Write-Log "Failed to update Komac GitHub token" -Level WARNING
            }
        }
        catch {
            Write-Log "Exception updating Komac token $_" -Level WARNING
        }
    }
    ...
    Write-Log "Running Komac dry-run with arguments [update $WingetId --version $NewVersion --dry-run --output $OutputFolder with $($InstallerUrls.Count) URLs]" -Level INFO
    ...
    Write-Log "Submitting updated manifest to create PR" -Level INFO
    ...
    try {
        ...
        $sanitizedOutput = $submitOutput -replace '(--token\s+)[a-zA-Z0-9_\-\.]{20,}', '$1***'
        $sanitizedOutput = $sanitizedOutput -replace '(token|access[_\s]token)(["\s=]+)[a-zA-Z0-9_\-\.]{20,}', '$1$2***'
        $sanitizedOutput = $sanitizedOutput -replace '(Bearer\s+)[a-zA-Z0-9_\-\.]{20,}', '$1***'
        
        # Log the sanitized Komac output
        if ($EnableVerboseMode) {
            Write-Log "Komac Submit Output $sanitizedOutput" -Level DEBUG -NoConsole
        }
...
##############################################################################
# Setup log rotation to avoid exposing secrets from previous runs
##############################################################################
function Initialize-LogFile {
    ...
    if (-not [string]::IsNullOrEmpty($logDir) -and -not (Test-Path $logDir)) {
        try {
            ...
            Write-Host "Warning: Could not create log directory. $_" -ForegroundColor Yellow
        }
    }
    ...
    try {
        if (Test-Path $LogFilePath) {
            # Optionally keep backups (up to 5)
            $backupPath = "$LogFilePath.bak"
            if (Test-Path $backupPath) {
                Remove-Item -Path $backupPath -Force | Out-Null
            }
            Rename-Item -Path $LogFilePath -NewName "$LogFilePath.bak" -Force | Out-Null
        }
        
        # Create a new log file with header
        $header = @"
# Winget Manifest Updater Log
# Started: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
# --------------------------------------------------

"@
        Set-Content -Path $LogFilePath -Value $header -Encoding UTF8 -Force | Out-Null
        
        return $true
    }
    catch {
        Write-Host "Error: Failed to initialize log file. $_" -ForegroundColor Red
        return $false
    }
}

##############################################################################
# Securely handle token parameters
##############################################################################
function Test-TokenProvided {
    param(
        [string]$Token,
        [string]$TokenName
    )
    ...
    if (-not [string]::IsNullOrWhiteSpace($Token)) {
        Write-Host "$TokenName provided and will be used." -ForegroundColor Green
        return $true
    }
    else {
        Write-Host "$TokenName not provided." -ForegroundColor Yellow
        return $false
    }
}
...
# Safely check if tokens are provided without logging them
Test-TokenProvided -Token $GitHubToken -TokenName "GitHub Token" | Out-Null
Test-TokenProvided -Token $gptKey -TokenName "OpenAI Key" | Out-Null
...
foreach ($wingetId in $wingetIds) {
    ...
    # Parse PR URL if available - completely rewritten for safety
    if ($updateResult -match 'https://github\.com/microsoft/winget-pkgs/pull/\d+') {
        # Extract the PR URL directly from the updateResult using the regex match
        $prUrl = $matches[0]  # This will be the actual URL
        $summary.PRs += @{
            PackageId = $wingetId
